## Learning objectives

- Explain the Takings Clause and the difference between eminent domain and regulatory takings.
- Trace the "public use" cases from *Berman* to *Kelo*.
- Distinguish the per se takings rules (*Loretto*, *Lucas*, *Cedar Point*) from the *Penn Central* balancing test.
- Apply the *Nollan*/*Dolan* nexus and proportionality tests to an exaction, including after *Koontz* and *Sheetz*.
- Recognize the procedural cases on temporary takings, moratoria, the "parcel as a whole," and access to federal court.

## Key concepts

> This lesson explains the doctrine at the level the exam tests. It isn't legal advice.

### The Takings Clause

The Fifth Amendment ends: "nor shall private property be taken for public use, without just compensation." Through the Fourteenth Amendment it applies to state and local governments. The clause does two things: it permits government to take property (**eminent domain**), and it requires **just compensation**, usually fair market value, when it does.

A property owner who believes a regulation has effectively taken their property, without any formal condemnation, can sue for compensation. This is called **inverse condemnation**.

### Eminent domain and "public use"

When government condemns property outright, the main legal question is whether the taking is for a **public use**. The Court has read that phrase broadly as "public purpose."

- ***Berman v. Parker*** **(1954)** upheld a Washington, D.C., urban renewal plan that took a non-blighted department store as part of clearing a blighted area. The Court said the public welfare is broad and includes aesthetic values, and that it's within the legislature's power to decide a community should be beautiful as well as healthy.
- ***Hawaii Housing Authority v. Midkiff*** **(1984)** upheld taking land from large landowners and transferring it to tenants to break up concentrated ownership.
- ***Kelo v. City of New London*** **(2005)** held, 5–4, that taking private homes for a comprehensive **economic development** plan that would transfer land to private developers satisfied the public use requirement. The backlash was immediate. Most states responded by passing laws or constitutional amendments restricting eminent domain for economic development.

### Regulatory takings

A regulation that doesn't physically take anything can still be a taking if it goes too far.

- ***Pennsylvania Coal Co. v. Mahon*** **(1922)**: Justice Holmes wrote that property may be regulated to a certain extent, but "if regulation goes too far it will be recognized as a taking." This is the origin of the regulatory takings doctrine.

Most regulatory takings claims are decided under a balancing test:

- ***Penn Central Transportation Co. v. New York City*** **(1978)** upheld New York's landmark designation of Grand Central Terminal, which blocked a tower above it. The Court weighed three factors:
  1. the **economic impact** of the regulation on the owner;
  2. the extent of interference with **distinct investment-backed expectations**; and
  3. the **character of the government action** (a physical invasion weighs more heavily than an adjustment of benefits and burdens for the common good).

  The Court looked at the **parcel as a whole**, not just the airspace, and noted the owner could still earn a reasonable return, and could transfer development rights to nearby sites.

### Per se (categorical) takings

In a few situations, a taking is automatic, with no balancing:

- ***Loretto v. Teleprompter Manhattan CATV Corp.*** **(1982)**: a **permanent physical occupation** authorized by government is a taking, no matter how small. The case involved cable boxes attached to an apartment building.
- ***Lucas v. South Carolina Coastal Council*** **(1992)**: a regulation that deprives land of **all economically beneficial use** is a taking unless the use was already prohibited by "background principles" of the state's property and nuisance law. Lucas had bought beachfront lots, and a later law barred building on them.
- ***Cedar Point Nursery v. Hassid*** **(2021)**: a regulation that grants third parties a right to **physically access** private property (here, union organizers entering farms for limited hours) is a per se physical taking, even though the access isn't continuous.

### Clarifications and procedure

- ***First English Evangelical Lutheran Church v. County of Los Angeles*** **(1987)**: if a regulation is a taking, the government must pay compensation for the period it was in effect, a **temporary taking**, even if the regulation is later repealed.
- ***Tahoe-Sierra Preservation Council v. Tahoe Regional Planning Agency*** **(2002)**: a temporary **development moratorium** (about 32 months) was *not* a per se taking. Moratoria are judged under *Penn Central*, looking at the parcel over time as a whole.
- ***Palazzolo v. Rhode Island*** **(2001)**: buying property *after* a regulation took effect doesn't automatically bar a takings claim.
- ***Lingle v. Chevron U.S.A.*** **(2005)**: the old test of whether a regulation "substantially advances" a legitimate interest belongs to **due process**, not takings. Takings analysis focuses on the burden on the owner.
- ***Murr v. Wisconsin*** **(2017)** addressed the **"denominator problem"**: which property counts as "the parcel" when measuring a regulation's impact. The Court used a multi-factor test and treated two adjacent, commonly owned lots that state and local law had merged as a single parcel.
- ***Knick v. Township of Scott*** **(2019)** held that a property owner may bring a takings claim in **federal court** right away, without first seeking compensation in state court. This overruled part of *Williamson County* (1985).

### Exactions

An **exaction** is a condition on a development approval that requires the developer to dedicate land, build improvements, or pay money. Because an exaction demanding land would be a taking if imposed directly, the Court requires a close fit between the condition and the project's impacts.

- ***Nollan v. California Coastal Commission*** **(1987)**: there must be an **essential nexus** between the condition and a legitimate state interest the project affects. Requiring a public beach-access easement as a condition of rebuilding a house failed, because the easement had nothing to do with the house's effect on visual access to the ocean.
- ***Dolan v. City of Tigard*** **(1994)**: the condition must be **roughly proportional**, in nature and extent, to the project's impact, based on an **individualized determination**. The city failed to show why a public greenway, rather than a private one, and a bike path were needed in proportion to the store expansion's impacts.
- ***Koontz v. St. Johns River Water Management District*** **(2013)** extended *Nollan*/*Dolan* to **permit denials** based on an owner's refusal to accept a condition, and to **monetary exactions** (demands for money in place of land).
- ***Sheetz v. County of El Dorado*** **(2024)** held that *legislatively* adopted fees, such as fees set by a general schedule, aren't exempt from the *Nollan*/*Dolan* tests simply because a legislature set them. The Court left open how the tests apply to fees assessed on a class of properties.

Many state courts had long used a similar **rational nexus** test for impact fees. See [Capital planning and public finance](/aicp/lessons/capital-planning-and-finance).

### Takings cases at a glance

| Case | Year | Holding in brief |
|---|---|---|
| *Pennsylvania Coal v. Mahon* | 1922 | Regulation that goes "too far" is a taking |
| *Berman v. Parker* | 1954 | Broad public purpose, including aesthetics, supports eminent domain |
| *Penn Central* | 1978 | Three-factor balancing test; parcel as a whole |
| *Loretto* | 1982 | Permanent physical occupation is a per se taking |
| *First English* | 1987 | Compensation owed for temporary takings |
| *Nollan* | 1987 | Exactions need an essential nexus |
| *Lucas* | 1992 | Total loss of economic use is a per se taking (with a nuisance exception) |
| *Dolan* | 1994 | Exactions must be roughly proportional |
| *Palazzolo* | 2001 | Later buyers can still bring takings claims |
| *Tahoe-Sierra* | 2002 | Moratoria aren't per se takings |
| *Kelo* | 2005 | Economic development can be a public use |
| *Lingle* | 2005 | "Substantially advances" is a due process test |
| *Koontz* | 2013 | Nexus and proportionality apply to denials and money demands |
| *Murr* | 2017 | Multi-factor test for defining the parcel |
| *Knick* | 2019 | Takings claims can go straight to federal court |
| *Cedar Point Nursery* | 2021 | Government-granted physical access is a per se taking |
| *Sheetz* | 2024 | Legislative fees aren't exempt from *Nollan*/*Dolan* |

## Key terms

- **Eminent domain**: The government's power to take private property for public use with just compensation.
- **Just compensation**: Payment for taken property, usually fair market value.
- **Inverse condemnation**: An owner's suit for compensation when government action effectively takes property without formal condemnation.
- **Regulatory taking**: A regulation so burdensome it is treated as a taking.
- **Per se taking**: A taking found automatically, without balancing.
- **Investment-backed expectations**: An owner's reasonable expectations when acquiring or investing in property.
- **Denominator problem**: The question of which property is "the parcel" when measuring a regulation's impact.
- **Exaction**: A condition requiring land, improvements, or money in exchange for development approval.
- **Essential nexus**: The required connection between an exaction and the project's impact.
- **Rough proportionality**: The required fit between the size of an exaction and the size of the impact.
- **Moratorium**: A temporary halt on certain development approvals while regulations are studied.

## Real-world examples

**Defending a path dedication.** A city wants a 20-foot multi-use path easement from each parcel along a corridor as it redevelops. To meet *Nollan* and *Dolan*, staff prepare a study linking the trips each type of development generates to the need for the path, and scale the requirement to each project's share of that demand. The city documents that analysis in each approval.

**A fee schedule after *Sheetz*.** A county's traffic impact fees are set by ordinance for all new homes. After *Sheetz*, the county attorney recommends reviewing the fee study to make sure the fees still reflect the demand each category of development actually creates.

**Designing a moratorium.** While it rewrites its floodplain rules, a town adopts an 18-month moratorium on new permits in the floodplain. Under *Tahoe-Sierra*, it isn't a per se taking, but the town still limits its duration and scope, ties it to a specific study, and allows hardship exemptions to keep it defensible under *Penn Central*.

## Summary

The Takings Clause permits eminent domain for public use with just compensation, and courts read "public use" broadly, from *Berman* through *Kelo*. Regulations can be takings too: *Loretto*, *Lucas*, and *Cedar Point* set per se rules, and most other claims go through *Penn Central*'s three-factor balancing of the parcel as a whole. *First English*, *Tahoe-Sierra*, *Palazzolo*, *Lingle*, *Murr*, and *Knick* refine the doctrine and its procedure. Exactions must have an essential nexus (*Nollan*) and rough proportionality (*Dolan*), tests that also cover permit denials and money demands (*Koontz*) and legislatively set fees (*Sheetz*).
