          ...
          {ITEMS.map((_, idx) => {
            ...
            return (
              <button
                ...
                style={{ aspectRatio: '1', borderRadius: 9, border: `1px solid ${f ? RUST : a ? GREEN : T.line}`, background: f ? RUST : a ? GREEN : T.surf, color: f || a ? '#FFFFFF' : T.mute, fontSize: 13, fontWeight: 600, fontFamily: 'inherit', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2 }}
              >
                <span>{idx + 1}</span>
                {f && <FlagIcon size={8} />}
