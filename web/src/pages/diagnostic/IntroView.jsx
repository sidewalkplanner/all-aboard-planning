...
const statCard = { background: '#FFFFFF', padding: '18px 20px' };
const statNum = { fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 26, fontWeight: 700, letterSpacing: '-0.02em' };
const statLabel = { fontSize: 13.5, color: '#646A85', marginTop: 2 };
...
export default function IntroView(s) {
  ...
  return (
    <section style={{ maxWidth: 820, margin: '0 auto', padding: '64px 24px 40px' }}>
      ...
      <p style={{ fontSize: 18, lineHeight: 1.6, color: '#454A63', margin: '18px 0 0', maxWidth: '60ch' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', gap: 1, background: '#E4E6F0', border: '1px solid #E4E6F0', borderRadius: 14, overflow: 'hidden', margin: '34px 0 0' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 26, margin: '40px 0 0' }}>
        <div>
          ...
          <p style={{ fontSize: 15, lineHeight: 1.65, color: '#454A63', margin: 0 }}>Sit down with a basic calculator and about two hours. You can move forward and back freely, flag anything, and leave items blank &mdash; blanks are scored as incorrect but reported separately, because a run of skips is itself a finding.</p>
        </div>
        <div>
          <h3 style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 19, fontWeight: 700, margin: '0 0 8px' }}>Tag your confidence</h3>
          <p style={{ fontSize: 15, lineHeight: 1.65, color: '#454A63', margin: 0 }}>Each item has a Confident / Unsure toggle. It is optional, but it is what lets the report separate a lucky guess from real command &mdash; and flag what you are confidently wrong about.</p>
        </div>
        <div>
          ...
          <p style={{ fontSize: 15, lineHeight: 1.65, color: '#454A63', margin: 0 }}>APA does not publish a passing percentage and the real exam is scaled. The bands here are study triage. Nothing in your report tells you whether you would pass.</p>
        </div>
      </div>

      {s.dSaved && (
        <div style={{ margin: '38px 0 0', border: '1px solid #D79A2B', background: '#FCF0DB', borderRadius: 14, padding: '20px 22px' }}>
          <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#8A6420' }}>Attempt in progress</div>
          <p style={{ fontSize: 15.5, lineHeight: 1.6, color: '#5A4A1E', margin: '8px 0 16px' }}>{savedNote}</p>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
            <Hoverable
              style={{ background: '#8A6420', color: '#FFFBF2', border: 'none', padding: '13px 24px', borderRadius: 10, fontSize: 15.5, fontWeight: 700 }}
            ...
            <Hoverable
              style={{ background: 'none', border: '1px solid #C9A65E', color: '#8A6420', padding: '13px 20px', borderRadius: 10, fontSize: 15.5, fontWeight: 600 }}
              hoverStyle={{ border: '1px solid #8A6420' }}
              onClick={s.startDiag}
            >
              Start over
            </Hoverable>
      ...
      <div style={{ margin: '38px 0 0', background: '#FFFFFF', border: '1px solid #E4E6F0', borderRadius: 14, padding: '20px 22px' }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'center', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 240 }}>
            <div style={{ fontSize: 16, fontWeight: 700, letterSpacing: '-0.01em' }}>Show answers as I go</div>
            <p style={{ fontSize: 14.5, lineHeight: 1.6, color: '#646A85', margin: '6px 0 0', maxWidth: '60ch' }}>
          ...
          <Hoverable
            style={{ fontSize: 13.5, fontWeight: 600, padding: '9px 16px', borderRadius: 9, fontFamily: 'inherit', border: `1px solid ${s.dReveal ? '#D79A2B' : '#D2D6E6'}`, background: s.dReveal ? '#FCF0DB' : '#FFFFFF', color: s.dReveal ? '#8A6420' : '#3A3F57' }}
      ...
      <div style={{ display: 'flex', gap: 14, alignItems: 'center', margin: '28px 0 0', flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 14, color: '#646A85' }}>Your place is saved as you go and held for 7 days.</span>
      </div>
    </section>
  );
}
