    ...
    <section style={{ maxWidth: 1180, margin: '0 auto', padding: '56px 24px 80px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 16 }}>
        {DOMAINS.map((d) => (
          <Hoverable
            key={d.name}
            style={{ textAlign: 'left', background: '#FFFFFF', border: '1px solid #E4E6F0', borderRadius: 16, padding: 22, display: 'flex', flexDirection: 'column', gap: 14 }}
          ...
          >
            ...
            <div style={{ marginTop: 'auto', width: '100%' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#646A85', marginBottom: 6 }}>
                <span>{countInBank(d.name)} questions</span><span>{d.mastery}% correct</span>
              </div>
